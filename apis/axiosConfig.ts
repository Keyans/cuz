...
//是否是单体应用
const isAone = import.meta.env.VITE_IS_AONE && import.meta.env.VITE_IS_AONE.toLowerCase() === 'true'
//单体应用矫正正则
const aoneUrlCorrectRegex = /\/?.*?\//
//url矫正函数
const urlCorrect = (currentUrl: undefined | string) => {
    return !currentUrl ? currentUrl : isAone ? currentUrl.replace(aoneUrlCorrectRegex, '/') : currentUrl
}
// 是否正在刷新的标记
...
instance.interceptors.request.use(
    (config) => {
        ...
        const { platformId, id,token } = useShopInfoStore().getterShopInfo
...
const refreshingFn = async () => {
    ...
    try {
        isRefreshing = true
        await useUserStore().login({ refreshToken, account: '', password: '', type: 'refreshToken' })
        requests.forEach((cd: (t: string) => any) => cd(useUserStore().userToken!))
        requests = []
        isRefreshing = false
    } catch (error) {
